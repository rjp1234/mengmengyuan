...
import com.mengmengyuan.common.config.QiniuConfig;
...
public class FileLoadUtils {
     ...
     * @author renjianping
     */
    private static void fileUpload(String localFilePath, String key) {
     ...
     * @author renjianping
     */
    private static void streamUpload(InputStream inputStream, String key) {
        // 构造一个带指定Zone对象的配置类
        Configuration cfg = new Configuration(Zone.zone0());
        UploadManager uploadManager = new UploadManager(cfg);
        // 获取七牛秘钥配置与空间配置
     ...
     * 
     */
    public static String QIniuupload(String sourceType, InputStream sourceIn, String sourceName) {
        String source = Global.getConfig("domain");
        int year = Calendar.getInstance().get(Calendar.YEAR);
        int month = Calendar.getInstance().get(Calendar.MONTH) + 1;
        int day = Calendar.getInstance().get(Calendar.DAY_OF_MONTH);
